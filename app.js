...
// Fix: Add types for custom properties on the window object for Google Maps API
declare global {
    interface Window {
        mapApiLoaded: Promise<any>;
        google: any;
        mapApiError: any;
    }
}
...
// --- API Configuration ---
const ApiContext = createContext<{ ai: GoogleGenAI | null, apiKey: string | null, isApiConfigured: boolean }>({ ai: null, apiKey: null, isApiConfigured: false });
...
const KnowledgeIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
...
const LeadsIconSolid = (props: React.SVGProps<SVGSVGElement>) => (
...
const EventsIconSolid = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" viewBox="0 0 20 20" fill="currentColor" {...props}>
...
const ChefToolsIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
...
const InvoicingIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
...
const OneDriveIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
const GCalendarIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
const safeApiCall = async (action: (ai: GoogleGenAI) => Promise<any>, ai: GoogleGenAI | null) => {
    if (!ai) {
...
const geocodeAddress = async (address: string, apiKey: string | null) => {
    try {
        if (!apiKey) {
...
const generateLeads = (ai: GoogleGenAI | null, location: string, clientType: string, serviceType: string) => {
    const schema = {
        type: Type.ARRAY,
        items: {
            type: Type.OBJECT,
...
// Fix: Add types to parameters and make systemInstruction optional to match usage
const getAIResponse = (ai: GoogleGenAI | null, prompt: string, systemInstruction?: string) => {
    return safeApiCall((ai) => {
        // Fix: Explicitly type config object to allow adding properties
...
const extractSearchTerm = (ai: GoogleGenAI | null, text: string) => {
    return safeApiCall((ai) => {
        return ai.models.generateContent({
            model: "gemini-2.5-flash",
...
const generateMenu = (ai: GoogleGenAI | null, serviceType: string, clientType: string, covers: string, dietaryRestrictions: string[], cuisine: string) => {
    const schema = {
        type: Type.OBJECT,
        properties: {
...
const generateEventPlan = (ai: GoogleGenAI | null, details: any) => {
    const schema = {
        type: Type.OBJECT,
        properties: {
...
const generateImageFromPrompt = async (ai: GoogleGenAI | null, prompt: string) => {
    if (!ai) {
        throw new Error("AI client is not initialized. Please configure the API key in Settings.");
    }
    try {
...
// Fix: Add Props and State types for ErrorBoundary class component
...
const Dashboard = ({ leads, events }: {leads: any[], events: any[]}) => {
  ...
  const leadFunnel = leads.reduce((acc: {[key: string]: number}, lead) => {
...
const Leads = ({ leads, onLeadsGenerated, showToast }: { leads: any[], onLeadsGenerated: (leads: any[]) => void, showToast: (message: string, type: string) => void }) => {
    ...
    const handleGenerateLeads = async () => {
        ...
        try {
            ...
            setIsGeocoding(true);
            const geocodedLeads = await Promise.all(results.map(async (lead: any) => {
...
const MenuGenerator = () => {
  ...
  const MenuCard = ({ title, course }: {title: string, course: {name: string, description: string}}) => (
...
const EventPlanner = ({ onEventSaved, showToast }: { onEventSaved: (event: any) => void, showToast: (message: string, type: string) => void }) => {
    const { ai, isApiConfigured } = useContext(ApiContext);
    ...
    const handleSaveEvent = () => {
        if (!plan) return;
        // Fix: Add explicit type annotation for newEvent to resolve type inference issue.
        const newEvent: { name: string, date: string, guests: number } = {
            name: plan.eventName,
...
const IntegrationCard = ({ icon, name, description, onConnect }: { icon: React.ReactNode, name: string, description: string, onConnect: (name: string, connected: boolean) => void }) => {
...
const Integrations = ({showToast}: {showToast: (message: string, type: string) => void}) => {
...
const SocialMedia = ({showToast}: {showToast: (message: string, type: string) => void}) => {
    const { ai, isApiConfigured } = useContext(ApiContext);
...
const Settings = ({ showToast, onApiConfigChange }: { showToast: (message: string, type: string) => void, onApiConfigChange: () => void }) => {
...
const Profile = ({ showToast }: { showToast: (message: string, type: string) => void }) => {
...
const NavItem = ({ icon, label, view, activeView, onClick }: { icon: React.ReactNode, label: string, view: string, activeView: string, onClick: (view: string) => void }) => {
